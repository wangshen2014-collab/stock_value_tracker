...
for _w in PE_WINDOWS:
    PE_ANALYSIS_RAW_COLUMNS.extend([
        f"PE_{_w}D_Count", f"PE_{_w}D_Min", f"PE_{_w}D_Max", f"PE_{_w}D_Mean", f"PE_{_w}D_Median",
        f"PE_{_w}D_Std", f"PE_{_w}D_P10", f"PE_{_w}D_P90", f"PE_{_w}D_Current_ZScore",
        f"PE_{_w}D_Current_Percentile", f"Value_{_w}D_Min", f"Value_{_w}D_Max", f"Value_{_w}D_Mean",
        f"Value_{_w}D_Lower_1Std", f"Value_{_w}D_Upper_1Std", f"Value_{_w}D_P10", f"Value_{_w}D_P90",
    ])
...
def get_last_price(ticker: yf.Ticker, info: dict[str, Any]) -> float | None:
    for key in ["currentPrice", "regularMarketPrice", "previousClose"]:
        ...
        if price:
            return price
    try:
        price = safe_float(ticker.fast_info.get("last_price"))
        if price:
            return price
    except Exception:
        pass
...
def get_ten_year_yield() -> float | None:
    ...
    try:
        hist = yf.Ticker("^TNX").history(period="10d", auto_adjust=False)
        raw = safe_float(hist["Close"].dropna().iloc[-1]) if not hist.empty else None
        if raw is None:
            LOGGER.warn("10Y Treasury yield unavailable")
            return None
        yield_value = raw / 1000.0 if raw > 20 else (raw / 100.0 if raw > 1 else raw)
        LOGGER.info(f"10Y Treasury yield: {fmt_pct(yield_value)}")
        return yield_value
    except Exception as exc:
        LOGGER.error("Fetch 10Y Treasury yield", exc)
        return None
...
def collect_one_stock(symbol: str, ten_year_yield: float | None) -> dict[str, Any]:
    ...
    try:
        ...
        LOGGER.error(f"{symbol} yfinance info", exc)
        info = {}

    price = get_last_price(ticker, info)
...
def build_report(tickers: list[str]) -> pd.DataFrame:
    ...
    for symbol in tickers:
        try:
            ...
            LOGGER.error(f"Build main snapshot for {symbol}", exc)
            row = {col: None for col in RAW_COLUMNS}
            row.update({"Ticker": symbol, "Company": f"ERROR: {exc}", "10Y Yield": ten_year_yield})
            rows.append(row)
...
def format_for_display(raw_df: pd.DataFrame) -> pd.DataFrame:
    out = raw_df.copy()
    for col in ["TTM PE", "Forward PE", "PEG", "PS"]:
        out[col] = out[col].apply(lambda x: fmt_num(x, 2))
    for col in ["EP", "Forward EP", "ERP"]:
        out[col] = out[col].apply(lambda x: fmt_pct(x, 2))
...
def sec_get_json(url: str) -> Any | None:
    try:
        response = requests.get(url, headers={"User-Agent": sec_user_agent(), "Accept-Encoding": "gzip, deflate"}, timeout=20)
        if response.status_code != 200:
            LOGGER.warn(f"SEC request failed HTTP {response.status_code}: {url}")
            return None
        return response.json()
    except Exception as exc:
        LOGGER.error(f"SEC request {url}", exc)
        return None
...
def quarter_from_record(record: dict[str, Any]) -> str | None:
    fp = str(record.get("fp", "")).upper()
    if fp in {"Q1", "Q2", "Q3", "Q4"}:
        return fp
    frame = str(record.get("frame", "")).upper()
    for quarter in ["Q1", "Q2", "Q3", "Q4"]:
        if quarter in frame and "YTD" not in frame:
            return quarter
    return None
...
def get_sec_ttm_eps_history(symbol: str) -> pd.DataFrame:
    ...
    for record in records:
        value = safe_float(record.get("val"))
        ...
        if value is None or filed is None or end is None:
            continue
        fy = int(record.get("fy") or end.year)
        fp = str(record.get("fp", "")).upper()
        ...
        if quarter in {"Q1", "Q2", "Q3", "Q4"} and is_quarter:
            key = (fy, quarter)
            old = quarters.get(key)
            if old is None or filed >= old["Filed"]:
                quarters[key] = {"FY": fy, "Quarter": quarter, "Filed": filed, "EPS": value, "Fiscal Order": fiscal_order(fy, quarter)}
        elif (fp == "FY" or "10-K" in form) and is_annual:
            old = annuals.get(fy)
            if old is None or filed >= old["Filed"]:
                annuals[fy] = {"FY": fy, "Filed": filed, "Annual EPS": value}

    for fy, annual in annuals.items():
        first_three = [quarters.get((fy, q), {}).get("EPS") for q in ["Q1", "Q2", "Q3"]]
        if all(v is not None for v in first_three) and (fy, "Q4") not in quarters:
            quarters[(fy, "Q4")] = {
                "FY": fy,
                "Quarter": "Q4",
                "Filed": annual["Filed"],
                "EPS": annual["Annual EPS"] - sum(first_three),
                "Fiscal Order": fiscal_order(fy, "Q4"),
            }
...
def get_yfinance_ttm_eps_history(ticker: yf.Ticker, symbol: str) -> pd.DataFrame:
    ...
    try:
        ...
        LOGGER.error(f"{symbol} Yahoo earnings dates", exc)
        earnings = None
    ...
    eps_col = next((col for col in df.columns if "eps actual" in str(col).lower() or "reported eps" in str(col).lower()), None)
    if eps_col is None:
...
def get_price_history_for_pe(ticker: yf.Ticker, symbol: str) -> pd.DataFrame:
    ...
    try:
        ...
        LOGGER.error(f"{symbol} price history", exc)
        hist = pd.DataFrame()
...
def merge_prices_with_eps(price_df: pd.DataFrame, eps_df: pd.DataFrame, symbol: str, method_name: str) -> pd.DataFrame:
    ...
    eps["Report Date"] = normalize_date_series(eps["Report Date"])
    prices = prices.sort_values("Date")
    eps = eps.sort_values("Report Date")
    LOGGER.info(f"{symbol}: merge {method_name}: price Date dtype={prices['Date'].dtype}, EPS Report Date dtype={eps['Report Date'].dtype}, price rows={len(prices)}, EPS rows={len(eps)}")
    try:
        merged = pd.merge_asof(
            prices,
            eps[["Report Date", "Point-in-Time TTM EPS"]],
            left_on="Date",
            right_on="Report Date",
            direction="backward",
        )
    except Exception as exc:
        LOGGER.error(f"{symbol} merge_asof {method_name}", exc)
        return pd.DataFrame(columns=["Date", "Close", "Historical PE"])
...
def add_window_stats(row: dict[str, Any], window: int, pe_series: pd.Series, current_pe: float | None, current_ttm_eps: float | None) -> None:
    prefix, value_prefix = f"PE_{window}D", f"Value_{window}D"
    pe = pe_series.dropna()
    pe = pe[pe > 0]
    count = int(len(pe))
    ...
    if current_pe is not None and vals["Std"] and vals["Std"] > 0:
        row[f"{prefix}_Current_ZScore"] = (current_pe - vals["Mean"]) / vals["Std"]
    if current_pe is not None:
        row[f"{prefix}_Current_Percentile"] = float((pe <= current_pe).sum() / count)
    if current_ttm_eps is not None and current_ttm_eps > 0:
        for key in ["Min", "Max", "Mean", "P10", "P90"]:
            row[f"{value_prefix}_{key}"] = safe_float(vals[key] * current_ttm_eps)
        row[f"{value_prefix}_Lower_1Std"] = max(0.0, (vals["Mean"] - vals["Std"]) * current_ttm_eps)
        row[f"{value_prefix}_Upper_1Std"] = (vals["Mean"] + vals["Std"]) * current_ttm_eps
...
def collect_one_pe_analysis(symbol: str, base_row: pd.Series, as_of_date: str) -> dict[str, Any]:
    ...
    for window in PE_WINDOWS:
        window_df = pe_df[pe_df["Date"] >= as_of_ts - pd.Timedelta(days=window)]
        add_window_stats(row, window, window_df["Historical PE"], current_pe, current_ttm_eps)
        LOGGER.info(f"{symbol}: {window}D PE observations={row.get(f'PE_{window}D_Count')}, PE range={fmt_range(row.get(f'PE_{window}D_Min'), row.get(f'PE_{window}D_Max'))}")
    if max([safe_float(row.get(f"PE_{window}D_Count")) or 0 for window in PE_WINDOWS]) < MIN_PE_OBSERVATIONS:
...
def build_pe_analysis(raw_df: pd.DataFrame, as_of_date: str) -> pd.DataFrame:
    ...
    for _, base_row in raw_df.iterrows():
        ...
        try:
            ...
            LOGGER.error(f"PE analysis for {symbol}", exc)
            if LOGGER.verbose:
                traceback.print_exc()
            rows.append(empty_pe_analysis_row(symbol, str(base_row.get("Company", "N/A")), f"ERROR: {exc}"))
...
def format_pe_analysis_for_display(pe_df: pd.DataFrame) -> pd.DataFrame:
    ...
    for _, row in pe_df.iterrows():
        rows.append({
            ...
            "Current PE": fmt_num(row.get("Current TTM PE"), 2),
            "EPS Used": fmt_num(row.get("Current TTM EPS"), 2),
            "90D PE Range": fmt_range(row.get("PE_90D_Min"), row.get("PE_90D_Max"), 2),
            "90D PE Std": fmt_num(row.get("PE_90D_Std"), 2),
            "90D PE Z": fmt_num(row.get("PE_90D_Current_ZScore"), 2),
            "90D Value Range": fmt_range(row.get("Value_90D_Min"), row.get("Value_90D_Max"), 2, True),
            "90D Normal Value": fmt_range(row.get("Value_90D_P10"), row.get("Value_90D_P90"), 2, True),
            "180D PE Range": fmt_range(row.get("PE_180D_Min"), row.get("PE_180D_Max"), 2),
            "180D PE Std": fmt_num(row.get("PE_180D_Std"), 2),
            "180D PE Z": fmt_num(row.get("PE_180D_Current_ZScore"), 2),
            "180D Value Range": fmt_range(row.get("Value_180D_Min"), row.get("Value_180D_Max"), 2, True),
            "180D Normal Value": fmt_range(row.get("Value_180D_P10"), row.get("Value_180D_P90"), 2, True),
...
def zscore_class(value: float | None) -> str:
    if value is None or pd.isna(value):
        return "neutral"
    return "good" if value <= -1.0 else ("okay" if value <= 1.0 else "bad")
...
def html_cell(column: str, display_value: str, raw_value: Any) -> str:
    ...
    return f'<td><span class="pill {metric_class(column, safe_float(raw_value))}">{html.escape(display_value)}</span></td>'
...
def build_main_table_html(raw_df: pd.DataFrame, display_df: pd.DataFrame) -> str:
    ...
    for idx, display_row in display_df.iterrows():
        ...
        rows.append("<tr>" + "".join(html_cell(col, str(display_row[col]), raw_row[col]) for col in DISPLAY_COLUMNS) + "</tr>")
...
def build_pe_table_html(pe_raw_df: pd.DataFrame, pe_display_df: pd.DataFrame) -> str:
    ...
    for idx, display_row in pe_display_df.iterrows():
        ...
        rows.append("<tr>" + "".join(pe_html_cell(col, str(display_row[col]), raw_row) for col in PE_DISPLAY_COLUMNS) + "</tr>")
...
def generate_html_report(raw_df: pd.DataFrame, display_df: pd.DataFrame, pe_raw_df: pd.DataFrame, pe_display_df: pd.DataFrame, tickers: list[str], as_of_date: str, generated_at: str) -> str:
    ...
    ticker_badges = " ".join(f'<span class="ticker-badge">{html.escape(ticker)}</span>' for ticker in tickers)
...
def save_history(raw_df: pd.DataFrame, as_of_date: str, generated_at: str, data_dir: str, export_excel: bool = True) -> tuple[Path, Path | None]:
    ...
    csv_path = Path(data_dir) / "valuation_history.csv"
    xlsx_path = Path(data_dir) / "valuation_history.xlsx"
    today = raw_df.copy()
    ...
    try:
        ...
        LOGGER.error("Main Excel export", exc)
        return csv_path, None
...
def save_pe_history(pe_raw_df: pd.DataFrame, as_of_date: str, generated_at: str, data_dir: str, export_excel: bool = True) -> tuple[Path, Path | None]:
    ...
    Path(data_dir).mkdir(parents=True, exist_ok=True)
    csv_path = Path(data_dir) / "pe_valuation_history.csv"
    xlsx_path = Path(data_dir) / "pe_valuation_history.xlsx"
    ...
    try:
        ...
        LOGGER.error("PE Excel export", exc)
        return csv_path, None
...
def save_daily_reports(raw_df: pd.DataFrame, display_df: pd.DataFrame, pe_raw_df: pd.DataFrame, pe_display_df: pd.DataFrame, tickers: list[str], output_dir: str, as_of_date: str, generated_at: str) -> tuple[Path, Path, Path, Path]:
    ...
    snapshot_path = daily_dir / f"stock_snapshot_{as_of_date}.csv"
    pe_snapshot_path = daily_dir / f"pe_valuation_snapshot_{as_of_date}.csv"
    latest_path = Path(output_dir) / "latest.html"
    raw_df.to_csv(snapshot_path, index=False)
    pe_raw_df.to_csv(pe_snapshot_path, index=False)
    ...
    return html_path, snapshot_path, pe_snapshot_path, latest_path
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--quiet", action="store_true", help="Reduce progress logs. Errors and warnings are still printed.")
...
def main() -> None:
    ...
    try:
        ...
        html_path, snapshot_path, pe_snapshot_path, latest_path = save_daily_reports(raw_df, display_df, pe_raw_df, pe_display_df, tickers, args.output_dir, as_of_date, generated_at)
        history_csv, history_xlsx = save_history(raw_df, as_of_date, generated_at, args.data_dir, not args.no_excel)
        pe_history_csv, pe_history_xlsx = save_pe_history(pe_raw_df, as_of_date, generated_at, args.data_dir, not args.no_excel)
        LOGGER.step("Output files")
        print(f"Saved daily HTML report:   {html_path}")
        print(f"Saved latest HTML copy:     {latest_path}")
        print(f"Saved daily CSV snapshot:   {snapshot_path}")
        print(f"Saved PE CSV snapshot:      {pe_snapshot_path}")
        print(f"Updated history CSV:        {history_csv}")
        print(f"Updated history Excel:      {history_xlsx if history_xlsx else 'skipped or failed'}")
        print(f"Updated PE history CSV:     {pe_history_csv}")
        print(f"Updated PE history Excel:   {pe_history_xlsx if pe_history_xlsx else 'skipped or failed'}")
        if args.open_browser:
            LOGGER.step("Opening HTML report in default browser")
            webbrowser.open(html_path.resolve().as_uri())
    except Exception as exc:
        LOGGER.error("Fatal run failure", exc)
        if LOGGER.verbose:
            traceback.print_exc()
        sys.exit(1)
    finally:
